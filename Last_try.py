...
class all_orders_builder:
    ...
    def get_all_orders(self):
        ...
        list = [
            [self.__red_rows_number,   const.RED,   new_order_builder(self.__red_rows_number, const.ID_START_VALUE, const.DONE, const.RED).get_order()],
            [self.__green_rows_number, const.GREEN, new_order_builder(self.__green_rows_number, int(start_string_id, 16) + const.ID_INCREMENT *  self.__green_rows_number, const.DONE, const.GREEN).get_order()],
            [self.__blue_rows_number,  const.BLUE,  new_order_builder(self.__blue_rows_number, int(start_string_id, 16) + const.ID_INCREMENT * (self.__green_rows_number + self.__blue_rows_number), const.FILL, const.BLUE).get_order()]
            ]            
...
class changed_order_builder(builder):
...
class new_order_builder(builder):
    ...
    def get_order(self):
        ...
        order_status = status(self.__prev_status, self.__index, self.__zone).get_field()
        order.add(order_status)
...
def main():
    for order in all_orders_builder(12, 12, 12).get_all_orders():
        print(tuple(order))
